...
try {
	...
	function consigueFranjaDIP($db){
			...
			$sqlCatPor = "SELECT '09' || printf('%04d', P.id_seccion) || substr(P.tipo_casilla, 1, 3) || '00' as CLAVE_CASILLA,
            ...
            P.representantes_votaron as TOTAL_REP_PARTIDO_CI_VOTARON, P.votacion_total as TOTAL_VOTOS_SACADOS,
                ...
                (CASE WHEN P.votos_part_8 is null and P.inconsistencia in (6,7) THEN 'SIN DATO' 
				 wHEN P.votos_part_8 is null and P.inconsistencia in (4,11,12,13,14) THEN 'SIN ACTA' 
				 WHEN P.votos_part_8 is null and P.inconsistencia in (1,2) THEN 'ILEGIBLE '
				 ELSE P.votos_part_8 end) as 'CSP',       
                ...
                P.boletas_sob as 'TOTAL_VOTOS_ASENTADOS', P.votacion_total  as 'TOTAL_VOTOS_CALCULADO',
            C.lista_nominal as 'LISTA_NOMINAL', P.representantes_votaron as 'REPRESENTANTES_PP_CI', CASE WHEN P.inconsistencia = 1 THEN
            ...
            CASE WHEN A.md5_img_dmr IS NULL THEN 'N/A' ELSE A.md5_img_dmr END as 'CODIGO_INTEGRIDAD',
            A.fechaRecepcion as 'FECHA_HORA_ACOPIO', P.fecha_alta as 'FECHA_HORA_CAPTURA',
            P.fecha_modif as 'FECHA_HORA_VERIFICACION',
...
//$v_votacion_sin_especiales = isset($r['votos_cand_no_reg']) ? $r['votacion_total'] - $r['votacion_sin_especiales'] : $r['votacion_total'];
...
////////////////////////	
	...
	fputcsv($f, $titulos, $delimiter);
		...
		$filename = $date."_PREP_DIP_LOC_CDMX.zip";
