...
try {
	...
	function consigueFranjaDIP($db){
			// Lamamos catálogo de porcentajes
			$sqlCatPor = "SELECT 
			printf('%03d', P.id_distrito) as DISTRITO_LOCAL,
			CASE
			...
			CASE
				WHEN substr(P.tipo_casilla, 1, 1) = 'S'
				and P.id_tipo_eleccion = 2 THEN 'A03EMR'
				WHEN substr(P.tipo_casilla, 1, 1) = 'S'
				and P.id_tipo_eleccion = 3 THEN 'A03ERP'
				...
				WHEN substr(P.tipo_casilla, 1, 1) = 'U'
				and P.id_tipo_eleccion = 2 THEN 'A05UEMR'
				WHEN substr(P.tipo_casilla, 1, 1) = 'U'
				and P.id_tipo_eleccion = 3 THEN 'A05UERP'
				ELSE 'A03'
			END as TIPO_ACTA,
			P.boletas_sob as TOTAL_BOLETAS_SOBRANTES,
			P.ciudadanos_votaron as PERSONAS_VOTARON,
			P.representantes_votaron as TOTAL_REP_PARTIDO_CI_VOTARON,
			P.votacion_total as TOTAL_VOTOS_SACADOS,
			(
				CASE
					WHEN P.votos_part_1 is null THEN 'SIN DATO'
					wHEN P.votos_part_1 is null THEN 'SIN ACTA'
					WHEN P.votos_part_1 is null THEN 'ILEGIBLE '
					ELSE P.votos_part_1
				end
			) as 'PAN',
			(
				CASE
					WHEN P.votos_part_2 is null THEN 'SIN DATO'
					wHEN P.votos_part_2 is null THEN 'SIN ACTA'
					WHEN P.votos_part_2 is null THEN 'ILEGIBLE '
					ELSE P.votos_part_2
				end
			) as 'PRI',
			(
				CASE
					WHEN P.votos_part_3 is null THEN 'SIN DATO'
					wHEN P.votos_part_3 is null THEN 'SIN ACTA'
					WHEN P.votos_part_3 is null THEN 'ILEGIBLE '
					ELSE P.votos_part_3
				end
			) as 'PRD',
			(
				CASE
					WHEN P.votos_part_4 is null THEN 'SIN DATO'
					wHEN P.votos_part_4 is null THEN 'SIN ACTA'
					WHEN P.votos_part_4 is null THEN 'ILEGIBLE '
					ELSE P.votos_part_4
				end
			) as 'PVEM',
			(
				CASE
					WHEN P.votos_part_5 is null THEN 'SIN DATO'
					wHEN P.votos_part_5 is null THEN 'SIN ACTA'
					WHEN P.votos_part_5 is null THEN 'ILEGIBLE '
					ELSE P.votos_part_5
				end
			) as 'PT',
			(
				CASE
					WHEN P.votos_part_6 is null THEN 'SIN DATO'
					wHEN P.votos_part_6 is null THEN 'SIN ACTA'
					WHEN P.votos_part_6 is null THEN 'ILEGIBLE '
					ELSE P.votos_part_6
				end
			) as 'MC',
			(
				CASE
					WHEN P.votos_part_7 is null THEN 'SIN DATO'
					wHEN P.votos_part_7 is null THEN 'SIN ACTA'
					WHEN P.votos_part_7 is null THEN 'ILEGIBLE '
					ELSE P.votos_part_7
				end
			) as 'MORENA',
			(
				CASE
					WHEN P.votos_part_8 is null THEN 'SIN DATO'
					wHEN P.votos_part_8 is null THEN 'SIN ACTA'
					WHEN P.votos_part_8 is null THEN 'ILEGIBLE '
					ELSE P.votos_part_8
				end
			) as 'CSP',
			(
				CASE
					WHEN P.votos_part_14 is null THEN 'SIN DATO'
					wHEN P.votos_part_14 is null THEN 'SIN ACTA'
					WHEN P.votos_part_14 is null THEN 'ILEGIBLE '
					ELSE P.votos_part_14
				end
			) as 'MORENA_PT_PVEM',
			(
				CASE
					WHEN P.votos_part_10 is null THEN 'SIN DATO'
					wHEN P.votos_part_10 is null THEN 'SIN ACTA'
					WHEN P.votos_part_10 is null THEN 'ILEGIBLE '
					ELSE P.votos_part_10
				end
			) as 'PAN_PRI_PRD',
			(
				CASE
					WHEN P.votos_part_11 is null THEN 'SIN DATO'
					wHEN P.votos_part_11 is null THEN 'SIN ACTA'
					WHEN P.votos_part_11 is null THEN 'ILEGIBLE '
					ELSE P.votos_part_11
				end
			) as 'PAN_PRI',
			(
				CASE
					WHEN P.votos_part_12 is null THEN 'SIN DATO'
					wHEN P.votos_part_12 is null THEN 'SIN ACTA'
					WHEN P.votos_part_12 is null THEN 'ILEGIBLE '
					ELSE P.votos_part_12
				end
			) as 'PAN_PRD',
			(
				CASE
					WHEN P.votos_part_13 is null THEN 'SIN DATO'
					wHEN P.votos_part_13 is null THEN 'SIN ACTA'
					WHEN P.votos_part_13 is null THEN 'ILEGIBLE '
					ELSE P.votos_part_13
				end
			) as 'PRI_PRD',
			(
				CASE
					WHEN P.votos_cand_no_reg is null THEN 'SIN DATO'
					WHEN P.votos_cand_no_reg is null THEN 'SIN ACTA'
					WHEN P.votos_cand_no_reg is null THEN 'ILEGIBLE '
					ELSE P.votos_cand_no_reg
				end
			) as 'NO_REGISTRADAS',
			(
				CASE
					WHEN P.votos_nulos is null THEN 'SIN DATO'
					WHEN P.votos_nulos is null THEN 'SIN ACTA'
					WHEN P.votos_nulos is null THEN 'ILEGIBLE'
					ELSE P.votos_nulos
				end
			) as 'NULOS',
			P.boletas_extraidas as 'TOTAL_VOTOS_ASENTADOS',
			P.votacion_total as 'TOTAL_VOTOS_CALCULADO',
...
// Fin funcion
	...
	//	$type = trim(htmlentities($_GET["type"]));
		...
		$itemRecords = consigueFranjaDIP($db);
	...
	//create a file pointer
		 $f = fopen('bd-diputacion.csv', 'w');
	
	//////////////////////////////////
	
	$delimiter = ",";
	$tipo_eleccion="DIPUTACION LOCALES";
	$blanks=array();
...
$q="SELECT id_tipo_eleccion, COUNT(C.id_distrito) AS cuantos, SUM(C.lista_nominal) AS ln, 
    ...
    FROM scd_votos P LEFT JOIN scd_casillas C 
        ON P.id_distrito = C.id_distrito 
        AND P.id_delegacion = C.id_delegacion 
        AND P.id_seccion = C.id_seccion 
        AND P.tipo_casilla = C.tipo_casilla WHERE id_tipo_eleccion in(2,3)";
...
///////////////////////////
// $q ="SELECT count(inconsistencia) as actas_fuera_catalogo FROM scd_votos where id_tipo_eleccion in(2,3) and inconsistencia=5";
...
// $q ="SELECT count(inconsistencia) as inconsistencia FROM scd_votos where id_tipo_eleccion in(2,3) and inconsistencia in (1,2,3,6,7)";
...
/////////////////esperadas
 ...
 if($r = $rows->fetchArray())
	{
   $esperadas=$r['esperadas']+44;  //actas fuera de catalogo
 }
...
$q="SELECT id_tipo_eleccion, COUNT(C.id_distrito) AS cuantos, SUM(C.lista_nominal) AS ln, 
        ...
        AND P.id_seccion = C.id_seccion 
        AND P.tipo_casilla = C.tipo_casilla
    where P.contabilizar='T' and  id_tipo_eleccion in(2,3)";
...
AND P.id_seccion = C.id_seccion 
AND P.tipo_casilla = C.tipo_casilla
where P.contabilizar='F' and id_tipo_eleccion in(2,3)";
...
////////////////////////	
	...
	fputcsv($f, $titulos, $delimiter);
		...
		$filename = $date."_SICODID_DIP_LOC_CDMX.zip";

		if ($zip->open($filename, ZipArchive::CREATE)!==TRUE) {
			exit("cannot open <$filename>\n");
		}

		$zip->addFile("bd-diputacion.csv","CDMX_DIP_LOC_2024.csv");
		$zip->addFile("CDMX_DIP_LOC_CANDIDATURAS.csv","CDMX_DIP_LOC_CANDIDATURAS_2024.csv");
		// $zip->addFile("LEEME_D.txt","leeme.txt");
