...
try {
	if($_SERVER['REQUEST_METHOD']=="GET"){
		...
		$res_catch = $db->query('SELECT tipo_casilla as [id], tipo_casilla as [name]  FROM scd_casillas where '.$filtro.' = '.$seleccion.' and id_seccion= '.$seccion.' order by id_seccion;');
		...
		while ($row_catch = $res_catch->fetchArray(SQLITE3_ASSOC))
		{
			...
			array_push($itemRecords["items"], $row_catch);
			$reg_cat++;
		}
