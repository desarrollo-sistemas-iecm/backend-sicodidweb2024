...
try {
	...
	function consigueFranjaALC($db){
            ...
            CASE WHEN substr(P.tipo_casilla, 1, 1) = 'S' THEN 'A04E'
                 ...
                 ELSE 'A04'
            END as TIPO_ACTA, P.boletas_sob as TOTAL_BOLETAS_SOBRANTES, P.ciudadanos_votaron as TOTAL_PERSONAS_VOTARON,
            ...
            P.votos_nulos as 'NULOS', P.votacion_total as 'TOTAL_VOTOS_ASENTADOS', (P.votacion_total + P.votos_nulos  + P.votos_cand_no_reg) as 'TOTAL_VOTOS_CALCULADOS',  C.lista_nominal as 'LISTA_NOMINAL', P.representantes_votaron as 'REPRESENTANTES_PP_CI', 
        ...
        END as 'CONTABILIZADAS', 'F' as 'MECANISMO_TRASLADO', A.md5_img_alc as 'CODIGO_INTEGRIDAD',
...
// Fin funcion
	...
	//	$type = trim(htmlentities($_GET["type"]));
		...
		$db = new SQLite3($_SERVER['DOCUMENTO_ROOT'].'../db/database.db3');
...
///////////////////////////
$q ="
SELECT count(inconsistencia) as actas_fuera_catalogo FROM prep_votos where id_tipo_eleccion=4 and inconsistencia=5";
...
$q ="
SELECT count(inconsistencia) as inconsistencia FROM prep_votos where id_tipo_eleccion=4 and inconsistencia in (1,2,3,4,6,7)";
...
SELECT sum(votacion_total) as votacion_total, sum(
	...
	end) as votacion_sin_especiales FROM prep_votos where id_tipo_eleccion=4 and contabilizar ='F'";
