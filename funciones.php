...
function getSQLdata($value_records, $type){
	...
	$group = " from scd_votos where ".$tipo_eleccion." and contabilizar='T' and id_distrito>0 and id_delegacion>0 group by ".$corteControl;
...
function getSQLsum($value_records, $type){
	...
	$group = " from scd_votos where contabilizar='T' and ".$tipo_eleccion." and ".$corteControl.">0 group by ".$corteControl;
...
function getRecordSetMap($type, $db){
		...
		//$group = " from scd_votos where id_tipo_eleccion=".$type." group by ".$corteControl;
...
//////////////////echo "<br><br>type ".$type.json_encode($value_records)."<br><br>";
...
//echo var_dump($sqlDataAll); die();
...

		{
			...
			////echo var_dump($value_records)."<br><br>";

			// ESTO HAY QUE PONER EN tmp[]

			/*

			foreach ($value_records as $registro1) {

					foreach ($registro1 as $clave => $valor) {

						if($valor["campo_votos"]=='total_votos_cc1')

							echo "<br>Valor: ".var_dump($valor)."<br><br>";
			...
			*/
			...
			//echo var_dump($rankedScores); die();
...
function getAvanceContabilizadaResumen($db, $type = 1){
		...
		$res_catch = $db->query("SELECT id_tipo_eleccion, contabilizar, count(C.id_distrito) as [cuantos], sum(C.lista_nominal) as ln, sum(votos_cand_no_reg) as [votos_cand_no_reg], sum(votos_nulos)  as [votos_nulos],
		 ...
		 sum(votacion_total) as votacion_total FROM scd_casillas C

		left join  scd_votos P

		on P.id_distrito = C.id_distrito and P.id_delegacion = C.id_delegacion and P.id_seccion = C.id_seccion and P.tipo_casilla = C.tipo_casilla 

		where validado='T' and contabilizar='T' 
...
function contarItemsPorParticipante($datos, $type = 2, $db) {
	...
	$resultado[] = [
			...
			"name"=>"Nombre(s) Apellido(s) Candidato 22",

			"color"=>$background_colors[intval(2)],
...
function getObtieneDistribucionPartidos ($db, $dttoAlc, $eleccion){
	...
	if( $eleccion == 1 ){ // Jefatura
		...
		$sql = "SELECT 
			...
			total_votos_cc9, votacion_total

			FROM scd_votos WHERE id_distrito = ".$dttoAlc." AND contabilizar = 'T' AND id_tipo_eleccion =".$eleccion;


	} else if( $eleccion == 2 ){ // DMR & RP
		...
		$sql = "SELECT 
			...
			FROM scd_votos WHERE id_distrito = ".$dttoAlc." AND contabilizar = 'T' AND (id_tipo_eleccion = 2 OR id_tipo_eleccion = 3)";

	} else if( $eleccion == 4 ) { // Alcaldía
		...
		$sql = "SELECT 
			...
			FROM scd_votos WHERE id_delegacion = ".$dttoAlc." AND contabilizar = 'T' AND id_tipo_eleccion =".$eleccion;
